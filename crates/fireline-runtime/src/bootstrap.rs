...
pub struct BootstrapConfig {
    ...
    pub stream_storage: Option<crate::runtime::StreamStorageConfig>,
...
pub struct BootstrapHandle {
    ...
    shared_terminal: crate::shared_terminal::SharedTerminal,
...
impl BootstrapHandle {
    pub async fn shutdown(mut self) -> Result<()> {
        ...
        crate::trace::emit_runtime_instance_stopped(
            &self.state_producer,
            &self.runtime_id,
            &self.runtime_name,
...
pub async fn start(config: BootstrapConfig) -> Result<BootstrapHandle> {
    ...
    let app = Router::new()
        .merge(crate::routes_acp::router(app_state))
    ...
    let persisted_spec = crate::runtime::PersistedRuntimeSpec::new(
        runtime_key.clone(),
        node_id.clone(),
        crate::runtime::CreateRuntimeSpec {
            runtime_key: Some(runtime_key.clone()),
            node_id: Some(node_id.clone()),
            provider: crate::runtime::RuntimeProviderRequest::Local,
            host: config.host,
    ...
    crate::trace::emit_runtime_spec_persisted(&state_stream_url, &persisted_spec)
        .await
        .context("emit runtime_spec_persisted from direct-host bootstrap")?;

    crate::trace::emit_runtime_instance_started(
        &state_producer,
        &runtime_id,
        &runtime_name,
        runtime_created_at,
