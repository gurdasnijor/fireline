...
- The right-hand **State Explorer** panel flips from *"Idle until a runtime is ready"* to showing the five tabs `sessions / turns / edges / chunks / connections`. The `sessions` tab is the default; it's empty because no ACP session has been opened yet, but the preload has completed against `ws://localhost:5173/v1/stream/fireline-harness-state` (proxied to `127.0.0.1:4437`).

**Primitive path:** the button calls `host.createSession({ agentCommand, metadata })` via `createFirelineHost` from `@fireline/client/host-fireline`. The satisfier POSTs to `/cp/v1/runtimes` — Vite proxies `/cp` to `http://127.0.0.1:4440` after stripping the prefix, so this hits the `fireline-control-plane` binary on port 4440. The control plane spawns a `fireline` binary child process on `127.0.0.1:4437` with the `fireline-testy-load` agent command, waits for the runtime to advertise `Ready`, and returns a `RuntimeDescriptor` to the browser. The browser wraps the `runtimeKey` into a `SessionHandle { id, kind: 'fireline' }` per the `Host.createSession` contract in [`./proposals/client-primitives.md`](./proposals/client-primitives.md) §Module 2.
...
**On screen:** each tab renders a live `useLiveQuery` against a different `@fireline/state` collection. The rows you see are materialized by TanStack DB from a durable-streams subscription to `ws://localhost:5173/v1/stream/fireline-harness-state` (proxied to the runtime's embedded durable-streams server on `:4437`).
...
**Primitive path:** TanStack DB live query → `createFirelineDB({ stateStreamUrl })` → `@durable-streams/state` subscription → `GET ws://localhost:5173/v1/stream/fireline-harness-state?live=sse` → vite proxy → `127.0.0.1:4437/v1/stream/...` → the durable-streams-server embedded inside the `fireline` binary (via `stream_host.rs`) → SSE back out through the same chain. The runtime itself writes into the same stream via its `DurableStreamTracer`, so there's a clean loop: *runtime projects ACP events → durable stream → @fireline/state materializes → TanStack DB renders → React reconciles*. **Same loop the `fireline-dashboard` TUI binary would use.**
