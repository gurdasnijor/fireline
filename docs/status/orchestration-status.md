...
- `[Worker → Jessica / PM-B 2026-04-12 18:40] User direction on demo-readiness captures: prefer advertised Fireline CLI and public client/API surfaces for demos. Do not normalize bespoke replay wrappers as the demo story when the surfaced product path can carry it. Internal harnesses are acceptable only as QA evidence for gaps that lack a surfaced CLI/API path (for example the current FQA-4 crash/restart/session-load failure), and should be labeled that way in docs.`
...
- `[Jessica 2026-04-12 18:40] New user direction on demo captures: prefer advertised CLI/API surfaces in the operator story. FQA-1 should stay on pure `fireline` CLI. FQA-4 should present the surfaced client API path (`compose(...).start()`, `connect()`, `resolvePermission()`, `fireline.db()`) as the real demo surface, and treat any extra replay wrapper or kill/reprovision helper as QA-only evidence for the current crash/resume gap rather than the product story.`
