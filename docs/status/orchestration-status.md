...
| **T2** | Tier A OCI end-to-end smoke on **Cloudflare Containers** | Target locked by user directive. Prove the w17/landed OCI image (38559b4 embedded-spec on 2c68794 portable OCI) on CF Containers: `wrangler deploy` (or equivalent CF tooling), ACP reachable, durable-streams sidecar persistent, prompt round-trips, container restart → session survives. Config under `deploy/cloudflare-containers/`. Review doc at `docs/reviews/smoke-tier-a-cloudflare-2026-04-12.md`. Reference: https://developers.cloudflare.com/containers/ | **Dispatched to w24** (redirected from J1; CF target locked by user override over original Fly recommendation). |
