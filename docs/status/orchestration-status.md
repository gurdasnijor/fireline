...
- **NEW — hosted Fireline deployment (preliminary direction, Architect 2026-04-12)** for PM queue item (7) `hosted-fireline-deployment.md`:
  - **(a) Runtime substrate for the Fireline HOST process: Fly.io-class Firecracker provider.** First-class long-lived stateful processes, persistent disks for durable-streams, multi-region placement for peer fleet, easy failure-domain reasoning against the TLA wake invariants. Rejected: Vercel serverless (fights the always-on ACP proxy model — sessions are long-lived SSE connections, not request/response). Rejected for MVP: raw Kubernetes (heaviest ops burden, overkill before scale demand). Fly/Railway/Render are all architecturally acceptable; MVP picks one concrete target.
  - **(b) Durable-streams hosting: dedicated service instance co-located with the Fireline host fleet,** persistent-disk backed, multi-tenant via namespace (`hosts:tenant-{id}`, `state/session/{session_id}`, etc.). Runs as a long-lived service on the same substrate as (a). Explicitly NOT embedded in any sandbox — durable streams must outlive every sandbox and every host-process crash per the plane-separation invariants and `SessionDurableAcrossRuntimeDeath`.
  - **(c) Cloud sandbox provider: Anthropic managed agents as PRIMARY** for the demo target; keep microsandbox + Docker as secondary providers for users needing full code-exec. Rationale: delegates sandbox lifecycle to a well-run managed backend, cleanest "one-file deploy to always-on cloud agent" narrative per `docs/demos/pi-acp-to-openclaw.md`, fastest path to the demo punchline. Multi-provider capability is preserved by the existing provider abstraction — this decision is a DEFAULT, not an exclusion.
