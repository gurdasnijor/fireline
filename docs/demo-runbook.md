...
4. **`@fireline/client` builds cleanly.**
   ...
   ```
   Expected output: `tsc` emits `dist/` with no errors. The `browser-harness` imports `@fireline/client` (for `compose`, `sandbox`, `middleware`, `agent`) and `@fireline/state` — if either fails to build, the Vite dev server in step 2 below will refuse to start with an import resolution error.
...
| `4437` | `target/debug/fireline` | Fireline sandbox process — exposes `/acp` (ACP WebSocket), `/v1/stream/*` (durable-streams-server), and `/healthz`. This is the sandbox the server spawns on demand when the browser calls `compose(sandbox, middleware, agent).start()`. | Fireline server (spawned as a child process, NOT by the dev server directly) |
...
**Why port 4437 is pinned:** the vite proxy config hardcodes `/acp`, `/v1`, and `/healthz` to `http://127.0.0.1:4437` (see `packages/browser-harness/vite.config.ts:16-25`). Under the new `compose().start()` API, the returned `SandboxHandle` carries the actual ACP and state endpoints (`handle.acp.url`, `handle.state.url`), so future harness versions can use those directly instead of relying on the vite proxy. For now, the vite proxy is still in the path and the demo relies on the sandbox landing on port 4437 in a clean dev environment. If that assumption breaks (another process squats on 4437 before the sandbox binds), the ACP WebSocket will fail to connect even though the sandbox itself is healthy. See §5c for the diagnosis ladder.

**Why port 4436 is the dev-server API:** the dev-server.mjs was written before the `compose()` API existed and originally fronted the entire runtime lifecycle. After the client API redesign, the browser creates sandboxes directly through the server via `compose(sandbox, middleware, agent).start({ serverUrl: '/cp' })`, and the 4436 API is mostly reduced to the **agent catalog endpoints** (`/api/agents` and `/api/resolve?agentId=...`). The legacy `/api/runtime` CRUD is still wired but no longer called by the browser-harness.
