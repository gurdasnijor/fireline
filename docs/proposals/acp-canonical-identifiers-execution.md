...
2. Do not advance to Phase `N+1` until Phase `N` is green in CI. Per the CI-first contention rules (see [`docs/status/orchestration-status.md § Contention rules`](../status/orchestration-status.md) — v2 tightened 2026-04-12 14:33), code-path codexes do **not** run cargo locally on the shared worktree at all; GitHub Actions runs `cargo check --workspace` + the full test suite and is the sole binding gate. Gate on `gh run list --limit 1 --json status,conclusion,url` → `gh run watch <run-id>` before advancing.
