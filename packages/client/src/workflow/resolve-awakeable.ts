...
// Process-local coordination only. Per Jeff's PR #28 review and DSV-01
// FirstResolutionWins, this map serializes same-process resolve/reject races
// for a single completion key, but it is not durable replay state and it does
// not coordinate across processes or restarts. `hasTerminalCompletion(...)`
// remains the authoritative durable check before any terminal append.
...
async function withResolveGuard<T>(
  ...
  // This is intentionally only an in-process critical section. DSV-01 replay
  // safety still comes from the durable pre-append read in
  // `hasTerminalCompletion(...)`, not from `resolveGuards`.
