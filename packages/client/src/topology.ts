import type { Harness, HarnessHandle, StartOptions } from './types.js'

export interface NamedTopology<Names extends string> {
  start(options: StartOptions): Promise<Record<Names, HarnessHandle<Names>>>
}

export interface FanoutTopology<Name extends string> {
  start(options: StartOptions): Promise<Array<HarnessHandle<Name>>>
}
...
export function peer<const T extends readonly Harness<string>[]>(
  ...
  return {
    async start(options) {
      ...
      ) as Record<T[number]['name'], HarnessHandle<T[number]['name']>>
    },
  }
}
...
export function pipe<const T extends readonly Harness<string>[]>(
  ...
  return {
    async start(options) {
      ...
      const handles: HarnessHandle<string>[] = []
