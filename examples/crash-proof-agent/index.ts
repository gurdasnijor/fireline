...
const acp1 = await connectAcp(first.acp, 'crash-proof-primary')
...
const acp2 = await connectAcp(second.acp, 'crash-proof-rescue')
...
const turns = await waitForRows(db.promptTurns, (rows) => rows.filter((row) => row.sessionId === sessionId && row.state === 'completed').length >= 2, 10_000)
