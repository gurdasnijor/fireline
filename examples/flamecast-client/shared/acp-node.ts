...
const unsupported = (name: string): never => {
  throw new Error(
    `ACP SDK requires stub client method '${name}'. For React/browser, use use-acp instead.`,
  );
};
    ...
    readable: new ReadableStream({
      start(controller) {
        socket.on("message", (data: RawData) =>
          controller.enqueue(JSON.parse(Buffer.isBuffer(data) ? data.toString("utf8") : String(data))),
        );
