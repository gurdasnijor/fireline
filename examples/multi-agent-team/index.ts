...
const researcher = await openNodeAcpConnection(handles.researcher.acp.url, 'team-researcher')
const writer = await openNodeAcpConnection(handles.writer.acp.url, 'team-writer')
const research = await researcher.connection.newSession({ cwd: '/workspace', mcpServers: [] })
await researcher.connection.prompt({ sessionId: research.sessionId, prompt: [{ type: 'text', text: 'Find the three biggest risks in this repo.' }] })
...
await writer.connection.prompt({ sessionId: write.sessionId, prompt: [{ type: 'text', text: `Turn this research into a one-page brief:\n${researchText}` }] })
